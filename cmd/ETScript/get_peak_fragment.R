...
bed_path <- cmd_args[7];
fragment_prefix <- cmd_args[8];
size_select <- as.numeric(cmd_args[9]);

peak_fragment <- paste(bed_path,"/",fragment_prefix,"_",threshold_txt,"_simple.bed",sep = "");
...
# d1+d2 <=size_select region
s_data$normReads <- s_data$reads / size_select
#s_data2 <- s_data[c(s_data$length>size_select),]

s_data$normReads[s_data$length<=size_select] <- s_data$reads[s_data$length<=size_select]/s_data$length[s_data$length<=size_select]
...
ns_data$normReads <- ns_data$reads / (ns_data$length-size_select)
ns_data2 <- ns_data[c(ns_data$length>size_select),]
